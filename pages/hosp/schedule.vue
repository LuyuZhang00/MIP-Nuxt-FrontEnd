<template>
  ...
  <div class="nav-container page-component">
    ...
    <div class="nav left-nav">
      ...
      <div class="nav-item ">
        <span class="v-link clickable dark" :onclick="'javascript:window.location=\'/hosp/detail / ' + hoscode + '\''">
          医院详情
        </span>
      </div>
      <div class="nav-item">
        <span class="v-link clickable dark" :onclick="'javascript:window.location=\'/hosp/notice / ' + hoscode + '\''">
          预约须知
        </span>
      </div>
        ...
        <div class="mt60">
          ...
          <div class="calendar-list-wrapper">
            ...
            <div :class="'calendar-item ' + item.curClass" style="width: 124px;"
              v-for="(item, index) in bookingScheduleList" :key="item.id" @click="selectDate(item, index)">
                ...
                <div class="right-wrapper">
                  ...
                  <div class="button-wrapper">
                    <div class="v-button" @click="booking(item.id, item.availableNumber)" :style="item.availableNumber == 0 || pageFirstStatus == -1 ? 'background-color: #7f828b;' : ''">
                      <span> 剩 余
                        <span class="number">{{ item.availableNumber }}</span></span>
                    </div>
                  </div>
                </div>
              </div>
  ...
  methods: {
    ...
    countDown(releaseTime, nowTime) {
      ...
      if (releaseTime > nowTime) {
        ...
        releaseTime = new Date(releaseDate.setDate(releaseDate.getDate(
        ) + 1)).getTime()
        //当前时间到明天放号时间的时长
        secondes = Math.floor((releaseTime - nowTime) / 1000);
      }
    ...
    convertTimeString(allseconds) {
      ...
      // 小时
      let hours = Math.floor((allseconds -
        (days * 60 * 60 * 24)) / (60 * 60));
      // 分钟
      let minutes = Math.floor((allseconds - (days * 60 * 60 * 24) -
        (hours * 60 * 60)) / 60);
      // 秒
      let seconds = allseconds - (days * 60 * 60 * 24) -
